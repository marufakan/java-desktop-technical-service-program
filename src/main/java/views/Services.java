...
public class Services extends Base {
    ...
    private void fncCmbStatusAdd(){
        cmbStatus.addItem(new ComboItem("Just arrived","0"));
        cmbStatus.addItem(new ComboItem("In repair","1"));
        cmbStatus.addItem(new ComboItem("Being repaired","2"));
        cmbStatus.addItem(new ComboItem("It was repaired","3"));
        cmbStatus.addItem(new ComboItem("It was delivered","4"));
    ...
    private void btnAddClick(ActionEvent e) {
        ...
        if(s !=null){
            ...
            if(status > 0){
                ...
                lblError.setText("Added service succes !");
            }
            else {
    ...
    private void btnUpdateServiceClick(ActionEvent e) {
        ...
        if (tblService.getSelectedRow() != -1) {
            ...
            int input = JOptionPane.showConfirmDialog(this, "Are you sure you want to update it?","Updated proceses",JOptionPane.YES_NO_OPTION);
            if(input==0){
                int cid = Integer.parseInt(txtCid.getText().toLowerCase(Locale.ROOT).trim());
                String title = txtTitle.getText().toLowerCase(Locale.ROOT).trim();
                String info = txtInfo.getText().toLowerCase(Locale.ROOT).trim();
    ...
    private void initComponents() {
        ...
        //---- btnAdd ----
        btnAdd.setIcon(new ImageIcon(getClass().getResource("/addCustomer.png")));
        btnAdd.setFocusable(false);
        btnAdd.addActionListener(e -> btnAddClick(e));

        //---- btnUpdate ----
        btnUpdate.setIcon(new ImageIcon(getClass().getResource("/updateCustomer.png")));
        btnUpdate.setFocusable(false);
        btnUpdate.addActionListener(e -> btnUpdateServiceClick(e));

        //---- btnDelete ----
        btnDelete.setIcon(new ImageIcon(getClass().getResource("/deleteCustomer.png")));
